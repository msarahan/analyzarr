...
# Copyright 2012 Michael Sarahan
#
# This file is part of  analyzarr. It is a fork of the original PIL dm3 plugin 
# written by Stefano Mazzucco.
#

# Unlike the rest of analyzarr, This file falls under the GPL.  Please consider this in any distribution you might do.

#  analyzarr is free software: you can redistribute it and/or modify
...
#from analyzarr.axes import DataAxis
#from analyzarr.exceptions import *
from libs.dm3reader import parseDM3

from libs.DictionaryBrowser import fsdict, DictionaryBrowser
import libs.binary_IO as binIO
...
read_char = binIO.read_byte # dm3 uses chars for 1-Byte signed integers
...
def read_infoarray(f):
    ...
    """
    infoarray_size = binIO.read_long(f,'big')
    infoarray = [binIO.read_long(f,'big') for index in xrange(infoarray_size)]
...
_data_type = {
    2 : (binIO.read_short, 2, 'h'),
    3 : (binIO.read_long, 4, 'l'),
    4 : (binIO.read_ushort, 2, 'H'), # dm3 uses ushorts for unicode chars
    5 : (binIO.read_ulong, 4, 'L'),
    6 : (binIO.read_float, 4, 'f'),
    7 : (binIO.read_double, 8, 'd'),
    8 : (binIO.read_boolean, 1, 'B'),
    9 : (read_char, 1, 'b'), # dm3 uses chars for 1-Byte signed integers
    10 : (binIO.read_byte, 1, 'b'),   # 0x0a
...
#                 print("Error. Could not write to file", fname)
        ...
        fsbrowser =  DictionaryBrowser(datadict_fs) # browsable dictionary'
        return fsbrowser
...
def file_reader(filename,record_by=None, order = None, data_id=1, 
    ...
    for tag in dm3.old_code_tags.items():
        node_valve(tag[0].split('.'), tag[1], tags)
